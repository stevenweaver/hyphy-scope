...
export function getFelAttributes(resultsJson) {
    ...
    const hasSrv = Object.values(content).some((partition) => partition?.some(([ds]) => ds > 0 && ds !== 1) || false);
    const hasCi = !!resultsJson["confidence interval"];
    const hasPositiveLRT = Object.values(content).some((partition) => partition?.some(([_, __, ___, ____, lrt]) => lrt > 0) || false);

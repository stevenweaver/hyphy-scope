...
export function getFelPlotOptions(hasPasmt: boolean): PlotOption[] {
  return [
...
export function getFelPlotDescription(plotType: string, pvalueThreshold: number): string {
 ...
 */
export function createPValuePlot(data: SiteData[], pvalueThreshold: number): any {
  const processedData = data.map(d => ({
    ...d,
 ...
 */
export function createRateDensityPlot(data: SiteData[]): any {
 ...
 */
export function createFelPlot(plotType: string, data: SiteData[], pvalueThreshold: number): any {
  switch (plotType) {
